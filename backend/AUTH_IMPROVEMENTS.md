...
describe("Auth Middleware", () => {
  it("should reject expired tokens", async () => {
    ...
    await expect(verifyToken(expiredToken, jwtSecret)).rejects.toThrow("Token has expired");
  });

  it("should reject blacklisted tokens", async () => {
    const token = generateValidToken();
    await blacklistToken(token.jti);
    await expect(verifyToken(token.raw, jwtSecret)).rejects.toThrow("Token has been revoked");
  });
